...
//  https://diyelectromusic.wordpress.com/2021/01/02/arduino-tone-step-sequencer/
//
*/

#define NUM_POTS 6 // Starting at A0
int SPEAKER = 9;
...
//#########################################################################
// Provide a list of the notes we want to be able to play
int notes[] = {
   NOTE_B0,
...
void loop() {
  ...
  // the note defined by the potentiometer for that point in the sequence. 
  ...
  if (potReading < MIN_POT_READING) {
    ...
    int pitch = map(potReading, MIN_POT_READING, 1023, 0, numNotes-1);

    tone (SPEAKER, notes[pitch]);

  // Print both the note and its index
    Serial.print("Note: ");
    Serial.print(notes[pitch]);
    Serial.print("\tIndex: ");
    Serial.print(pitch);
    Serial.print("\t");
  }
    // Move on to the next note after a short delay
    // 250 = 1/4 of a second so this is the same as 240 beats per minute
    // (1 second / 250ms = 4 notes per second)
    // 4 notes per second * 60 seconds = 240 beats per minute
    delay(250);
    playingNote++;
}
